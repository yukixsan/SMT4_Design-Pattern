...
### Core Game Classes
- **Map**:
  - Manages the game map (23x39 grid).
...
- **Ghost**:
  - Represents a ghost with state-based behavior.
...
2. **Compile**:
   ```bash
   g++ -I include src/*.cpp main.cpp -o pacman.exe

## Adding a New Ghost Type
To add a new ghost type (e.g., "Shadow" with icon S), you only need to modify the GhostFactory to include the new type in its random selection.

### Steps
Update GhostFactory.cpp:

Add the new icon to the icons array in createRandomGhost.
Adjust the random selection range to account for the new type.
## Example
#include "GhostFactory.h"

bool GhostFactory::seeded = false;

void GhostFactory::seedRandom() {
    if (!seeded) {
...
Ghost* GhostFactory::createRandomGhost(int x) {
    seedRandom();
    char icons[] = {'B', 'P', 'I', 'C', 'S'}; // Added 'S' for Shadow
    int typeIndex = rand() % 5; // Updated to 5 options
...
Compile and run the game with a higher difficulty (e.g., 3 or 4) to increase the chance of spawning the new ghost.
Look for the S icon on the map, behaving like other ghosts (wandering, chasing, etc.).

## Result
A new ghost type “Shadow” (S) is now part of the game, randomly spawned by the factory.
No changes to main.cpp or other classes were needed, demonstrating the Factory Pattern’s extensibility.
